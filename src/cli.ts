...
async function main() {
  ...
  if (values.help) {
    console.log('aisa-web-market serve [--install-skills --client codex|claude-code|hermes]\naisa-web-market setup|uninstall --client codex|claude-code|hermes [--home directory]\naisa-web-market status\nSet AISA_API_KEY in the server environment. setup prompts for a missing key and saves it in the client MCP configuration.'); return;
  }
  if (positionals.length > 1) throw Error('Unexpected positional argument.');
  if (command === 'status') { console.log(JSON.stringify({credential_configured: Boolean(process.env.AISA_API_KEY?.trim()), authentication: 'bearer', version: '0.1.1'})); return; }
  if (command === 'setup' || command === 'uninstall') {
    if (!values.client) throw Error('--client is required.');
    console.log(JSON.stringify(await install(values.client as Client, {home: values.home, remove: command === 'uninstall', resolveKey: command === 'setup' ? resolveSetupKey : undefined})));
